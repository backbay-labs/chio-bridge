...
async function client(transport:{url:string;token:string}){
 ...
 if(outcome.state==="completed"){
  const ack=await (await request({jsonrpc:"2.0",id:`ack:${id}`,method:"chio/acknowledge",params:outcome.delivery})).json();assert.equal(ack.result.acknowledged,true);
 }
 return response;}};
}
...
test("lost guest response cannot silently dispatch a replacement request", async () => {
 const f=await fixture();const transport=await startGatewayHttp(f.config);
 let dropped=false;
 const proxy=createServer(async(request,response)=>{
  let body="";for await(const chunk of request)body+=chunk;const message=JSON.parse(body);
  const upstream=await fetch(transport.url,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${transport.token}`,...(request.headers["mcp-session-id"]?{"Mcp-Session-Id":String(request.headers["mcp-session-id"])}:{})},body});
  const text=await upstream.text();
  if(message.method==="tools/call"&&!dropped){dropped=true;response.destroy();return;}
 ...
 await new Promise<void>(resolve=>proxy.listen(0,"127.0.0.1",resolve));const address=proxy.address();assert.ok(address&&typeof address!=="string");
 try{
  const c=await client({url:`http://127.0.0.1:${address.port}/mcp`,token:transport.token});
  await assert.rejects(()=>c.call(1,"original outcome lost to guest"));assert.equal(f.effects(),1);
  const replacement=JSON.parse((await c.call(2,"replacement must not execute")).result.content[0].text);
  assert.equal(replacement.state,"not_dispatched");assert.equal(f.effects(),1);
 }finally{proxy.closeAllConnections();await new Promise<void>(resolve=>proxy.close(()=>resolve()));await transport.close();await f.close();}
});


test("unconfirmed host delivery survives restart and only exact received proof permits continuation", async()=>{
 const f=await fixture();let transport=await startGatewayHttp(f.config);
 try{
  let c=await client(transport);
  const raw=await c.request({jsonrpc:"2.0",id:1,method:"tools/call",params:{name:"write_file",arguments:{path:"allowed.txt",content:"retained original"}}});
  await raw.text(); // Operator transport consumed it; do not acknowledge host delivery.
  await transport.close();transport=await startGatewayHttp(f.config);c=await client(transport);
  assert.equal(JSON.parse((await c.call(1,"new transport must not retry")).result.content[0].text).state,"not_dispatched");
  assert.equal(f.effects(),1);
  const path=join(f.config.journalDir,readdirSync(f.config.journalDir).find(name=>name.endsWith(".json"))!);
  const retained=JSON.parse(readFileSync(path,"utf8"));
  for(const field of ["requestId","requestHash","receiptId","resultHash","acknowledgement"]){
    const refused=await transport.acknowledgeDelivery({...retained.outcome.delivery,[field]:"forged"});assert.equal(refused.acknowledged,false);assert.equal(f.effects(),1);
  }
  const recovered=await transport.acknowledgeDelivery(retained.outcome.delivery);assert.equal(recovered.acknowledged,true);assert.equal(f.effects(),1);
